...
export class InvoicesService {
  ...
  findAll(creditCardNumber?: string) {
    let query = this.invoiceRepo.createQueryBuilder("invoice").select(["invoice.*"]);

    if (creditCardNumber) {
      query = query.leftJoin("invoice.credit_card", "credit_card").andWhere("credit_card.number = :creditCardNumber", {
        creditCardNumber
      });
    }
