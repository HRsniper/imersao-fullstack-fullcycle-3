...
const fetcher = (resource: string) => http.get(resource).then((res) => res.data);

const InvoicesListPage: NextPage<InvoicesListPageProps> = ({ creditCards }) => {
  const [creditCardNumber, setCreditCardNumber] = useState<string>(creditCards.length ? creditCards[0].number : "");

  const { data: invoices = [], error } = useSWR<Invoice[]>(`credit-cards/${creditCardNumber}/invoices`, fetcher, {
          ...
          <List>
            {invoices.map((invoice, key) => (
              <ListItem key={key} alignItems="flex-start">
                ...
                />
                <ListItemSecondaryAction>R$ {invoice.amount}</ListItemSecondaryAction>
              </ListItem>
            ))}
          </List>
        </Grid>
...
export const getStaticProps: GetStaticProps<InvoicesListPageProps> = async (context) => {
  try {
